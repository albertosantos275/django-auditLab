...
def compute_agregates(ref_sheet):
    ...
    by_person= tp01_agregates.agregate_by_person_type(ref_sheet,0,7,0, 25,2, person_types)
    # #ref_sheet, row_name, key_col, amount_col, 
    # #max_col_number,row_number, codes
    #by_countries = fdo03b_agregates.agregate_by_single_col(ref_sheet,'country',6,7,24,2, countries)
    by_province  = tp01_agregates.agregate_by_provinces(ref_sheet,5,7,0, 34,2, localidades)
    by_average  = tp01_agregates.agregate_by_average(ref_sheet,1,7,2,3,0,25,2)
...
def translate(ref_sheet):
    translate_row(ref_sheet,'user',1, 2,person_types )
