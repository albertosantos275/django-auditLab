...
def agregate_by_person_type(ref_sheet,type_col, amount_col, min_col_number, max_col_number,row_number, codes, orig_col,div_col): 
    data={
    }
    risk_data={
        # "Alto":{"total":0},
    ...
    for row in ref_sheet.iter_rows(min_row=row_number,
        ...
        filter_type= row[0].value
        orig_amount = str(row[orig_col].value).strip(' ')
        orig_amount = float(orig_amount)
        divisa =  str(row[div_col].value).strip(' ')
        # print(f"{orig_amount} {divisa}")
        # if filter_val!= filter_type:
...
def agregate_by_services(ref_sheet,service_col, amount_col,  min_col_number, max_col_number,row_number, codes, filter_val): 
    data={}
    total = 0
    risk_total =0
    phases={}
...
def agregate_by_provinces(ref_sheet,service_col, amount_col,  min_col_number, max_col_number,row_number, codes, filter_val): 
    data={}
    total = 0
    risk_total =0
    temp_risks={}
...
def agregate_by_economic_activity(ref_sheet, user_col, amount_col,  min_col_number, max_col_number,row_number, codes, user_codes): 
    data={
    ...
    for row in ref_sheet.iter_rows(min_row=row_number,
        ...
        user_id = row[user_col].value
        transaction = row[amount_col].value
        
        # print(f"{user_id} {transaction} ")
        activity = None
...
def agregate_by_average(ref_sheet, user_col, amount_col,name_col,other_col, min_col_number, max_col_number,row_number, filter_val): 
    ...
    for row in ref_sheet.iter_rows(min_row=row_number,
        ...
        transaction = str(row[amount_col].value).strip(' ')
        transaction = float(transaction)
        client_id = str(row[user_col].value).strip(' ')
...
def agregate_by_divisa(ref_sheet, row_name, key_col, amount_col, 
    max_col_number,row_number, codes,orig_col): 
    data={
    }
    total = 0
    def init_data(k, data):
    ...
    for row in ref_sheet.iter_rows(min_row=row_number,
        ...
        if key in codes.keys():
            ...
            data[key]["total"]+=transaction
            data[key]['amount']+=1
            data[key]["total_original"]+=orig_amount
            total+=transaction
        else:
            print(f"user type not found {key}")
