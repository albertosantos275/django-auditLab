...
def compute_agregates(ref_sheet):
    ...
    by_client= de15_agregates.agregate_by_client_type(ref_sheet,28,7,0, 50,2, client_types)
    by_person= de15_agregates.agregate_by_person_type(ref_sheet,2,7,0, 25,2, person_types)
    # #ref_sheet, row_name, key_col, amount_col, 
    # #max_col_number,row_number, codes
    by_vinculacion = de15_agregates.agregate_by_single_col(ref_sheet,'name',16,7,32,2, vinculacion_type)
    by_origin = de15_agregates.agregate_by_single_col(ref_sheet,'name',31,7,40,2, origin_resources)
    by_province  = de15_agregates.agregate_by_provinces(ref_sheet,17,7,0, 34,2, localidades)
    by_average  = de15_agregates.agregate_by_average(ref_sheet,1,7,3,4,0,25,2)
    
    
    #by_activity  = de13_agregates.agregate_by_economic_activity(ref_sheet,30,8,0, 40,2, economic_activities)
